/* global L */
// Ajoute la première source de tuiles qui répond (fallback automatique)
function addFallbackTiles(map){
  ...
  const tryNext = () => {
    ...
    const onErr  = () => setTimeout(() => { if (!loaded) tryNext(); }, 500);
    layer.on('load', onLoad); layer.on('tileerror', onErr); layer.addTo(map); return layer;
  };
  return tryNext();
}
